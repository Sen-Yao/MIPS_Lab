// 本代码是为了应付 2023.5.17 晚的微机原理实验操作考试准备的，基于实验 3 修改得到。
...
#include "xgpio_l.h"
#include "xtmrctr.h"
// #define RESET_VALUE 100000000 - 2  // 这里定义给计时器的时间常数。由于是减计数，因此得到 1 秒的延迟，需要写入 100 M - 2，如果要做到人眼显示，那 10000 差不多     
...
// short number_seg_code[16] = {0xc0, 0xf9,  0xa4, 0xb0, 0x99, 0x92, 0x82, 0xf8, 0x80, 0x90, 0x88, 0x83, 0xc6, 0xa1, 0x86, 0x8e};  
// 二维数组进行查询
...
void Initialize()
{
    ...
    //对定时器进行初始化
    /*
    Xil_Out32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET, Xil_In32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET) & ~XTC_CSR_ENABLE_TMR_MASK); // 写 TCSR，停止计数器
    Xil_Out32(XPAR_AXI_TIMER_0_BASSADDR + XTC_TLR_OFFSET, RESET_VALUE); // 写 TLR，预置计数器初值
    Xil_Out32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET, Xil_In32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET) | XTC_CSR_LOAD_MASK); // 写 TCSR，装载计数初值
    Xil_Out32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET, (XIL_IN32(XPAR_AXI_TIMER_0_BASEADDR + XTC_TCSR_OFFSET) & ~XTC_CSR_LOAD MASK) | XTC_CSR_ENABLE_TMR_MASK | XTC_CSR_AUTO_RELOAD_MASK|XTC_CSR_ENABLE_INT_MASK|XTC_CSR_DOWN_COUNT_MASK);
    */
...
for (index = 0; index<5; index++)
    ...
    {
        for(j=0; j<7; j++)  // 更新缓冲区
            segcode[j] = segcode[j+1];  // 将已存在显示缓冲区中间的段码全部左移一位
            segcode[7] = scancode[index][1];  // 将当前按键的段码放在最后
            break;
    }

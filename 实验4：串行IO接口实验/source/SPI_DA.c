...
int main()
{
    ...
    while(1)
    {
    	if(flag_Sw)  // 若拨动 switch 开关，则打印相关信息
        {
            ...
            xil_printf("Now the period is %fs\n\r", to_set_value * 4.096);
...
void SPIHandler()
{
    ...
    Xil_Out16(XPAR_AXI_QUAD_SPI_0_BASEADDR + XSP_DTR_OFFSET, volt & 0xfff);//启动 SPI 使能，产生时钟和片选信号。数字信号对应的有效位为低 12 位，因此在这里将volt 与 0xfff 相与之后再输出
    Xil_Out32(XPAR_AXI_INTC_0_BASEADDR + XIN_ISR_OFFSET, 0x00);
    Xil_Out32(XPAR_AXI_INTC_0_BASEADDR + XIN_IER_OFFSET, 0x07); // 写 1 清除中断标志位

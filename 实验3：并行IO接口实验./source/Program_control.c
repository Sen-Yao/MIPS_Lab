...
int main()
{
    ...
    Xil_Out8(XPAR_AXI_GPIO_2_BASEADDR + XGPIO_TRI_OFFSET, 0x1f); // 按键全部为 Input 模式
    Xil_Out16(XPAR_AXI_GPIO_0_BASEADDR + XGPIO_TRI2_OFFSET, 0x00);  // LED 全部为 Output 模式
    while(1) // 一直执行下面的内层 while 循环
    while ((Xil_In8(XPAR_AXI_GPIO_2_BASEADDR + XGPIO_DATA_OFFSET)&0x1f)!=0) // 循环检测按钮是否按下，按键没有按下时为低电平，因此只要任何一个按键按下，读入寄存器的值就不为 0，从而进入循环
    {
        button = Xil_In8(XPAR_AXI_GPIO_2_BASEADDR + XGPIO_DATA_OFFSET)&0x1f; // 读取按键的值
